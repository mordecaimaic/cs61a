...
def funception(func1, begin):
    ...
    """
    "*** YOUR CODE HERE ***"
    # 重要！！！！非常重要的一点就是，如果要把parent frame的"begin"当作参数传入的话，不能对"beigin“参数进行修改
    # 也就是如果进行begin += 1类似的操作的话，python的解释器会认为这个是local的变量，而不会往父级frame去寻找这个变量,会出现错误local variable
    # 正确的操作，使用 i = begin 进行操作
    # cs61a的作业介绍里面也做了很详细的说明的
    def function(n):
        i, total = begin, 1
        while i < n:
            total *= func1(i)
