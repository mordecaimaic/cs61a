...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def detect_word(s):
        split_text = remove_punctuation(s).lower().split()
        for key in subject: 
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    type_len, source_len, correct_num = len(typed_words), len(source_words), 0
    if type_len == 0 and source_len == 0:
        return 100.0
    elif type_len == 0:
        return 0.0
    elif source_len == 0:
        ...
        for i in range(source_len):
            if typed_words[i] == source_words[i]:
                correct_num += 1
    return 100 * correct_num / type_len
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        if limit_check(word):
            best_match = min(best_match, word, key = word_pass)
            find = True
    return best_match if find else typed_word
